import type { OptimizeTraceStep } from './DebugOptimizePanel'

export type DebugAnnealingTraceSliderProps = {
...
export function DebugAnnealingTraceSlider({
  steps,
  bestRestartIndex,
  ...
  return (
    <>
      <div className="mt-2 mb-1 flex flex-wrap items-center justify-between gap-2">
        ...
        >
          {showBestRoute ? 'ベスト候補を表示中' : 'ベスト候補に戻す'}
        </button>
      </div>
      {steps.length === 0 ? (
        ...
        <input
          ...
          className={[
            ...
            // WebKit: トラックはグラデで「塗り / ベース」を分離（accent は使わない）
            ...
            '[&::-webkit-slider-thumb]:bg-[var(--trace-fill)] [&::-webkit-slider-thumb]:shadow-sm',
            // Firefox
            '[&::-moz-range-track]:h-1.5 [&::-moz-range-track]:rounded-full [&::-moz-range-track]:bg-[var(--trace-track)]',
            '[&::-moz-range-progress]:h-1.5 [&::-moz-range-progress]:rounded-full [&::-moz-range-progress]:bg-[var(--trace-fill)]',
            ...
            '[&::-moz-range-thumb]:bg-[var(--trace-fill)] [&::-moz-range-thumb]:shadow-sm',
          ].join(' ')}
        />
      )}
      <p className={`font-mono text-[10px] ${showBestRoute ? 'text-amber-500' : 'text-stone-500'}`}>
        {showBestRoute ? (
          <>ベスト候補を表示中（試行 #{bestRestartIndex}）</>
        ) : displayStep ? (
          <>
            試行 #{selectedRestartIndex} · トレース {selectedTraceIndex + 1}/{steps.length} · step {displayStep.step_index} ·
            score={displayStep.score_total.toFixed(4)}
          </>
        ) : null}
      </p>
    </>
