...
- **優先度**: 実装が難しければ **優先度は低め**でよい。本アプリの主目的は **GPSアート作成**であり、立体交差の完全除外が **技術的に困難またはコスト過大**な場合、意図したスコープの例外として許容する（エッジケース）。

### B3 — 道路交差の幾何検出と split（製品差別化として重視）

**背景（なぜ必要か）**

OSM では、十字のように交わる縦道 `A–B` と横道 `C–D` が、**交点 E で共通ノードを持たず**、二つの独立した `way` として登録されていることが多い。このままグラフ化すると、交点 E を **グラフのノードとして表現できず**、例えば `**A–E–D` のようなジオメトリ**（交点で「曲がる」表現）が取れない。GPSアートのように **形**を道路に沿わせる用途では、交点での自由度が効く。
...
OSM では同じ道路・近い車線・部分的に重なる way が複数定義されることがある。GPSアート最適化では、これらを別々の道路として残すと、同じ場所を同じ場所として扱えず、局所的な back-and-forth や探索空間の増加につながる。
